...
will create the a node C</files/etc/hosts/4/ipaddr> if there is no matching
node, and a node already exists named C</files/etc/hosts/3>
...
=item B<AUGEAS_CREATEIFNOMATCH>

If set to anything but "0", this is equivalent to using the command-line
option C<--createifnomatch>
