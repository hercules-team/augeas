...
root=$abs_top_builddir/build/test-augprint
augeas_root=$abs_top_builddir/tests/test-augprint

export AUGEAS_LENS_LIB=$abs_top_builddir/lenses
# ------------- /etc/hosts ------------
mkdir -p $root/etc
AUGEAS_ROOT=$augeas_root augprint --pretty --verbose /etc/hosts > $root/etc.hosts.augtool
...
AUGEAS_ROOT=$augeas_root augprint --regexp=2 /etc/hosts > $root/etc.hosts.augtool
AUGEAS_ROOT=$root augtool --load-file=/etc/hosts -f  $root/etc.hosts.augtool --noload --autosave 1>/dev/null
...
for test_file in /etc/squid/squid.conf /etc/pam.d/system-auth ; do
  ...
  output=${test_file//\//.}
  output=${output#.}
	AUGEAS_ROOT=$augeas_root augprint $test_file > $root/${output}.augtool
